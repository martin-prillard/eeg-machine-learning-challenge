    ...
    * Weighting of the cost function
    ...
    def score(self, X):
        ...
        out : float
            The beta-divergence
        """
        return np.nansum(_betadiv(X, parafac(self.factors_), self.beta))
...
def nnrandn(shape):
    ...
    """
    #print shape
    #res = np.abs((np.random.randn(*shape)))
    #print res
    #print len(res)
    return np.abs(np.random.randn(*shape))


def build_model(X, n_components, n_iter, beta):

    # Create BetaNTF object with Euclidean (beta=2) distance
    beta_ntf = BetaNTF(X.shape, n_components=n_components, beta=beta, n_iter=n_iter,
                       verbose=True)
    ...
    print 'Resulting score', beta_ntf.score(X)
    ...
    model= parafac(beta_ntf.factors_)

    return model, beta_ntf
    # TODO useless ?
    """
    ...
    """
    """
    print 'Shape of total_model : ',model.shape

    model = matutils.Dense2Corpus(model, documents_columns=False)
